...
def string_to_dict(string) -> Dict[str, int]:
    ...
    if "+" in string:
        ...
        if plus_split[1] == "(Man)3(GlcNAc)2":
            ...
            raise ValueError(
                f"Unexpected trailing sequence {plus_split[1]}, expected (Man)3(GlcNAc)2"
            )
        return elements_count
...
def truncated_str_from_dict(dictionary):
    ...
    total_hexnac = dictionary.get("(HexNAc)", 0) + dictionary.get(
        "(GlcNAc)", 0)
    return f"{'H' + str(total_hex)}{'N' +str(total_hexnac)}" + \
...
def calc_theor_mono_mass(dictionary, cfg, prec=6, reducing_end=None) -> float:
    ...
    if reducing_end is not None:
        if reducing_end in cfg["reducing_end_tag_mono"].keys():
            ...
            raise ValueError(
                f"Invalid reducing end tag. Available tags:  {list(cfg['reducing_end_tag'].keys())}"
            )
...
def _calc_theor_mono_mass_adducts(dictionary,
                                  cfg,
                                  charge,
                                  reducing_end_mass,
                                  adduct_mass,
    ...
    return round((sum([
        dictionary[key] * cfg["mono_masses_underivatized"][key]
        for key in dictionary.keys()
    ]) + cfg["mono_masses_underivatized"]["H2O"] + reducing_end_mass +
                  adduct_mass) / charge, prec)


def calc_default_adducts_mono(dictionary, cfg, reducing_end, prec=4):
    reducing_end_tag_mass = 0.0
    if reducing_end:
        if reducing_end in cfg["reducing_end_tag_mono"].keys():
            reducing_end_tag_mass = cfg["reducing_end_tag_mono"][reducing_end]
        else:
            err = f"Invalid reducing end tag [{reducing_end}]. Available tags: {list(cfg['reducing_end_tag_mono'].keys())}"
            raise ValueError(err)
    adduct_ions = {
        i: _calc_theor_mono_mass_adducts(
            dictionary, cfg, j[1], reducing_end_tag_mass, j[0], prec=prec)
        for i, j in cfg["adducts"].items()
    }
    return adduct_ions
    ...
    if len(string) > 1:
        for ind, char in enumerate(string):
            if char.isalnum():
                ...
                raise ValueError(
                    f"Char {ind} invalid in {string}. File name must consist of letters, numbers '-' and '_'. Please provide a valid file name"
                )
    return filename
...
def check_text_input_consistency(string):
    ...
    for char in stripped_text:
        if char.isdigit() or char == ".":
            ...
            raise ValueError(
                f"Your input contains invalid character '{char}'; Input should consist of floating point numbers."
            )
